...
const getFeaturedProducts = async (req, res) => {
  try {
    ...
    res.json({ status: true, products: featuredProducts });
  } catch (err) {
    res.status(500).json({ status: false, message: err.message });
  }
};
...
const updateProduct = async (req, res) => {
  try {
    const { id } = req.params;
    // Make sure to include featured in update, or default to existing value
...
module.exports = {
  addProduct,
  getAllProducts,
  getFeaturedProducts, // <-- Export new function
  updateProduct,
  deleteProduct,
  getProductById,
};
